...
static size_t group_desc_count;      /* number of block group descriptors */
...
int ext2_block_used(uint32_t blkaddr) {
  ...
  // id of block group which has block 'blkaddr'
  size_t bg_id = (blkaddr - 1) / blocks_per_group;
  // index of `blkaddr' in this group
  size_t b_idx = (blkaddr - 1) % blocks_per_group;

  blk_t *blk = blk_get(0, group_desc[bg_id].gd_b_bitmap);
  used = ((*((uint8_t *)(blk->b_data + b_idx / 8))) >> (b_idx % 8)) & 1;
  blk_put(blk);
...
int ext2_inode_used(uint32_t ino) {
  ...
  // id of block group which has inode 'ino'
  size_t bg_id = (ino - 1) / inodes_per_group;
  // index of 'ino' in this group
  size_t i_idx = (ino - 1) % inodes_per_group;

  blk_t *blk = blk_get(0, group_desc[bg_id].gd_i_bitmap);
  used = ((*((uint8_t *)(blk->b_data + i_idx / 8))) >> (i_idx % 8)) & 1;
  blk_put(blk);
...
static int ext2_inode_read(off_t ino, ext2_inode_t *inode) {
  ...
  size_t bg_id = (ino - 1) / inodes_per_group;
  size_t g_id = (ino - 1) % inodes_per_group;

  // calculate offset
  ext2_groupdesc_t block_gd = group_desc[bg_id];
  size_t i_addr =
    (block_gd.gd_i_tables * BLKSIZE) + (g_id * sizeof(ext2_inode_t));
...
long ext2_blkaddr_read(uint32_t ino, uint32_t blkidx) {
  ...
  // number of pointers in blocks
  uint32_t single_cnt = BLK_POINTERS;
  uint32_t double_cnt = BLK_POINTERS * BLK_POINTERS;
  uint32_t triple_cnt = BLK_POINTERS * BLK_POINTERS * BLK_POINTERS;

  // max idx in blocks
  uint32_t single_max = single_cnt + EXT2_NDADDR;
  uint32_t double_max = double_cnt + single_max;
  uint32_t triple_max = triple_cnt + double_max;

  long addr = -1;

  // direct block
  if (blkidx < EXT2_NDADDR)
    addr = inode.i_blocks[blkidx];

  // single indirect
  else if (blkidx < single_max)
    addr = ext2_blkptr_read(inode.i_blocks[12], blkidx - EXT2_NDADDR);

  // double indirect
  else if (blkidx < double_max) {
    blkidx -= single_max;

    uint32_t ptr1 = ext2_blkptr_read(inode.i_blocks[13], blkidx / single_cnt);
    blkidx %= single_cnt;

    addr = ext2_blkptr_read(ptr1, blkidx);
  }

  // triple indirect
  else if (blkidx < triple_max) {
    blkidx -= double_max;

    uint32_t ptr1 = ext2_blkptr_read(inode.i_blocks[14], blkidx / double_cnt);
    blkidx %= double_cnt;

    uint32_t ptr2 = ext2_blkptr_read(ptr1, blkidx / single_cnt);
    blkidx %= single_cnt;

    addr = ext2_blkptr_read(ptr2, blkidx);
  }
...
int ext2_read(uint32_t ino, void *data, size_t pos, size_t len) {
  ...
  size_t block_idx = pos / BLKSIZE;

  while (len > 0) {
    blk_t *blk = blk_get(ino, block_idx);
    size_t offset = pos % BLKSIZE;

    size_t cnt = min(BLKSIZE - offset, len);

    if (blk == BLK_ZERO) { // hole
      memset(data, 0, cnt);
    } else {
      memcpy(data, blk->b_data + offset, cnt);
      blk_put(blk);
    }
...
int ext2_readdir(uint32_t ino, uint32_t *off_p, ext2_dirent_t *de) {
  ...
  *off_p += de_name_offset;
  ...
  // read name and terminate by NULL
...
int ext2_readlink(uint32_t ino, char *buf, size_t buflen) {
  ...
  if (buflen < 60) { // short links can be stored within inode
    memcpy(buf, inode.i_blocks, inode.i_size);
    buf[inode.i_size] = '\0';
  } else {
    assert(!ext2_read(ino, buf, 0, inode.i_size));
    buf[inode.i_size] = '\0';
  }
...
int ext2_lookup(uint32_t ino, const char *name, uint32_t *ino_p,
  ...
  uint32_t offset = 0;
  while (ext2_readdir(ino, &offset, &dirent)) {
    if (strcmp(name, dirent.de_name) == 0) {
      if (ino_p != NULL) {
        *ino_p = dirent.de_ino;
      }
      if (type_p != NULL) {
...
int ext2_mount(const char *fspath) {
  ...
  size_t gd_len = group_desc_count * sizeof(ext2_groupdesc_t);
  group_desc = malloc(gd_len);
  if (!ext2_read(0, group_desc, EXT2_GDOFF, gd_len))
    return 0;
